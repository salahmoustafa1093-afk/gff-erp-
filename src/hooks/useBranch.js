...
import { setCurrentBranch, setBranchList, setBranchLoading, setBranchError, } from '@/app/slices/branchSlice';
...
export function useBranch() {
    ...
    const updateBranch = useMutation({
        mutationFn: async (data) => {
            ...
            const response = await apiService.patch(`/branches/${id}`, updateData);
            return response;
        },
        onSuccess: (updatedBranch) => {
            const updatedList = branchList.map((b) => b.id === updatedBranch.id ? updatedBranch : b);
    ...
    // Computed
    const activeBranches = useMemo(() => branchList.filter((b) => b.isActive), [branchList]);
    const mainBranch = useMemo(() => branchList.find((b) => b.isMain), [branchList]);
    const currentBranchName = useMemo(() => currentBranch?.name || 'Select Branch', [currentBranch]);
    const currentBranchCode = useMemo(() => currentBranch?.code || '', [currentBranch]);
    return {
        // State
        currentBranch,
        branchList,
        ...
        switchBranch,
        refreshBranches,
        setCurrentBranch: useCallback((branch) => dispatch(setCurrentBranch(branch)), [dispatch]),
        // Mutations
