...
import { Visibility as ViewIcon, Print as PrintIcon, Payment as PaymentIcon, MoreVert as MoreIcon, Search as SearchIcon, Refresh as RefreshIcon, FileDownload as ExportIcon, } from '@mui/icons-material';
...
const PurchaseInvoicesPage = () => {
    ...
    const [paymentForm, setPaymentForm] = useState({ amount: '', paymentDate: new Date(), method: 'BANK_TRANSFER', reference: '', notes: '' });
    ...
    return (_jsx(LocalizationProvider, { dateAdapter: AdapterDateFns, children: _jsxs(Box, { sx: { p: 2 }, children: [_jsxs(Box, { sx: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }, children: [_jsx(Typography, { variant: "h4", fontWeight: "bold", color: "primary", children: "Purchase Invoices" }), _jsx(Button, { variant: "outlined", startIcon: _jsx(ExportIcon, {}), onClick: handleExport, children: "Export" })] }), _jsxs(Paper, { sx: { p: 2, mb: 2 }, children: [_jsxs(Box, { sx: { display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'center', mb: 2 }, children: [_jsx(TextField, { placeholder: "Search invoice # or supplier...", size: "small", value: searchQuery, onChange: (e) => setSearchQuery(e.target.value), sx: { minWidth: 260 }, slotProps: {
                                        input: {
                                            startAdornment: (_jsx(InputAdornment, { position: "start", children: _jsx(SearchIcon, { fontSize: "small" }) })),
                                        },
                                    } }), _jsxs(TextField, { select: true, size: "small", label: "Status", value: statusFilter, onChange: (e) => setStatusFilter(e.target.value), sx: { minWidth: 140 }, children: [_jsx(MenuItem, { value: "", children: "All Status" }), Object.entries(statusConfig).map(([key, { label }]) => (_jsx(MenuItem, { value: key, children: label }, key)))] }), _jsx(IconButton, { onClick: () => refetch(), size: "small", children: _jsx(RefreshIcon, {}) })] }), _jsx(DataGrid, { rows: invoices, columns: columns, rowCount: totalRows, loading: isLoading, paginationModel: paginationModel, onPaginationModelChange: setPaginationModel, onSortModelChange: setSortModel, pageSizeOptions: [10, 25, 50, 100], paginationMode: "server", sortingMode: "server", disableRowSelectionOnClick: true, onRowClick: (params) => navigate(`/purchases/invoices/${params.id}`) })] }), _jsxs(Menu, { anchorEl: actionAnchorEl, open: Boolean(actionAnchorEl), onClose: () => setActionAnchorEl(null), children: [_jsxs(MenuItem, { onClick: () => { selectedInvoice && navigate(`/purchases/invoices/${selectedInvoice.id}`); setActionAnchorEl(null); }, children: [_jsx(ViewIcon, { fontSize: "small", sx: { mr: 1 } }), " View"] }), _jsxs(MenuItem, { onClick: () => { selectedInvoice && window.open(`/purchases/invoices/${selectedInvoice.id}/print`, '_blank'); setActionAnchorEl(null); }, children: [_jsx(PrintIcon, { fontSize: "small", sx: { mr: 1 } }), " Print"] }), selectedInvoice && selectedInvoice.balance > 0 && selectedInvoice.status !== 'CANCELLED' && (_jsxs(MenuItem, { onClick: () => {
                                setPaymentForm({ ...paymentForm, amount: String(selectedInvoice.balance) });
                                setPaymentDialog(true);
                                setActionAnchorEl(null);
                            }, children: [_jsx(PaymentIcon, { fontSize: "small", sx: { mr: 1 } }), " Record Payment"] }))] }), _jsxs(Dialog, { open: paymentDialog, onClose: () => setPaymentDialog(false), maxWidth: "sm", fullWidth: true, children: [_jsx(DialogTitle, { children: "Record Payment" }), _jsxs(DialogContent, { children: [selectedInvoice && (_jsxs(Box, { sx: { mb: 2 }, children: [_jsxs(Typography, { variant: "body2", color: "text.secondary", children: ["Invoice: ", selectedInvoice.invoiceNumber] }), _jsxs(Typography, { variant: "body2", color: "text.secondary", children: ["Supplier: ", selectedInvoice.supplierName] }), _jsxs(Typography, { variant: "body1", fontWeight: "bold", color: "primary", children: ["Balance Due: ", formatCurrency(selectedInvoice.balance)] })] })), _jsxs(Grid, { container: true, spacing: 2, children: [_jsx(Grid, { size: { xs: 12, sm: 6 }, children: _jsx(TextField, { label: "Amount *", type: "number", fullWidth: true, size: "small", value: paymentForm.amount, onChange: (e) => setPaymentForm({ ...paymentForm, amount: e.target.value }), inputProps: { min: 0.01, step: 0.01 } }) }), _jsx(Grid, { size: { xs: 12, sm: 6 }, children: _jsx(DatePicker, { label: "Payment Date *", value: paymentForm.paymentDate, onChange: (val) => val && setPaymentForm({ ...paymentForm, paymentDate: val }), slotProps: { textField: { size: 'small', fullWidth: true } } }) }), _jsx(Grid, { size: { xs: 12, sm: 6 }, children: _jsx(TextField, { select: true, label: "Method *", fullWidth: true, size: "small", value: paymentForm.method, onChange: (e) => setPaymentForm({ ...paymentForm, method: e.target.value }), children: paymentMethods.map((m) => (_jsx(MenuItem, { value: m, children: m.replace('_', ' ') }, m))) }) }), _jsx(Grid, { size: { xs: 12, sm: 6 }, children: _jsx(TextField, { label: "Reference #", fullWidth: true, size: "small", value: paymentForm.reference, onChange: (e) => setPaymentForm({ ...paymentForm, reference: e.target.value }) }) })] })] }), _jsxs(DialogActions, { children: [_jsx(Button, { onClick: () => setPaymentDialog(false), children: "Cancel" }), _jsx(Button, { variant: "contained", onClick: () => selectedInvoice && paymentMutation.mutate({ invoiceId: selectedInvoice.id, data: paymentForm }), disabled: !paymentForm.amount || Number(paymentForm.amount) <= 0 || paymentMutation.isPending, children: paymentMutation.isPending ? 'Saving...' : 'Record Payment' })] })] })] }) }));
};
