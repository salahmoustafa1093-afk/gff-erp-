...
import {
  Box, Button, Chip, Grid, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Typography, Divider,
} from '@mui/material';
...
import { Supplier, CustomerStatementRow } from '../types';
...
const SupplierStatementPage: React.FC = () => {
  ...
  const handlePrint = () => {
    ...
    if (!printContent || !supplier) return;
      ...
      <Box sx={{ p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <IconButton onClick={() => navigate(`/suppliers/${id}`)}><BackIcon /></IconButton>
            <Typography variant="h4" fontWeight="bold" color="primary">Supplier Statement</Typography>
          </Box>
          ...
          <Paper sx={{ p: 3, mb: 3 }}>
            <Grid container spacing={3}>
              <Grid size={{ xs: 12, md: 6 }}>
                <Typography variant="h6" sx={{ mb: 1 }}>{supplier.name}</Typography>
                <Typography variant="body2" color="text.secondary">{supplier.code}</Typography>
                <Typography variant="body2" color="text.secondary">{supplier.address}{supplier.city ? `, ${supplier.city}` : ''}</Typography>
        ...
        <Paper sx={{ p: 2, mb: 3 }}>
          <Grid container spacing={2} alignItems="center">
            <Grid size={{ xs: 12, sm: 4 }}>
              <DatePicker label="From Date" value={fromDate} onChange={setFromDate} slotProps={{ textField: { size: 'small', fullWidth: true } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <DatePicker label="To Date" value={toDate} onChange={setToDate} slotProps={{ textField: { size: 'small', fullWidth: true } }} />
            </Grid>
            <Grid size={{ xs: 12, sm: 4 }}>
              <Button variant="contained" onClick={() => refetch()} fullWidth>Generate Statement</Button>
            </Grid>
          </Grid>
        </Paper>
        ...
        <Paper sx={{ p: 2 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
            ...
            <Box sx={{ textAlign: 'right' }}>
              ...
              <Typography variant="h6" fontWeight="bold" color={closingBalance > 0 ? 'error' : 'success'}>{formatCurrency(closingBalance)}</Typography>
            </Box>
          </Box>
          <Divider sx={{ mb: 2 }} />
          <TableContainer>
            <Table size="small">
              ...
              <TableBody>
                {rows.length === 0 && !isLoading && (
                  <TableRow><TableCell colSpan={7} align="center" sx={{ py: 4 }}><Typography color="text.secondary">No transactions</Typography></TableCell></TableRow>
                )}
                {rows.map((row, idx) => (
                  <TableRow key={idx} hover>
                    <TableCell>{format(parseISO(row.date), 'dd/MM/yyyy')}</TableCell>
                    <TableCell><Chip label={row.type} size="small" color={row.type === 'PAYMENT' ? 'success' : 'default'} /></TableCell>
                    <TableCell fontWeight="medium">{row.reference}</TableCell>
                    <TableCell>{row.description}</TableCell>
                    <TableCell align="right" sx={{ color: row.debit > 0 ? 'error.main' : 'inherit' }}>{row.debit > 0 ? formatCurrency(row.debit) : '-'}</TableCell>
                    <TableCell align="right" sx={{ color: row.credit > 0 ? 'success.main' : 'inherit' }}>{row.credit > 0 ? formatCurrency(row.credit) : '-'}</TableCell>
        ...
        <Box sx={{ display: 'none' }}>
          <div ref={printRef}>
            <div className="header"><h1>SUPPLIER STATEMENT</h1><p>{fromDate ? format(fromDate, 'dd/MM/yyyy') : ''} - {toDate ? format(toDate, 'dd/MM/yyyy') : ''}</p></div>
            {supplier && (
              <div className="supplier-info">
                <div><p><strong>{supplier.name}</strong></p><p>{supplier.code}</p><p>{supplier.address}{supplier.city ? `, ${supplier.city}` : ''}</p><p>{supplier.phone}</p></div>
                <div style={{ textAlign: 'right' }}><p>Opening Balance: {formatCurrency(openingBalance)}</p><p>Closing Balance: {formatCurrency(closingBalance)}</p></div>
              </div>
            )}
            <table>
              ...
              <tbody>
                {rows.map((row, idx) => (
                  <tr key={idx}><td>{format(parseISO(row.date), 'dd/MM/yyyy')}</td><td>{row.type}</td><td>{row.reference}</td><td>{row.description}</td>
                    <td className="debit">{row.debit > 0 ? formatCurrency(row.debit) : ''}</td>
                    <td className="credit">{row.credit > 0 ? formatCurrency(row.credit) : ''}</td>
                    <td>{formatCurrency(row.balance)}</td></tr>
                ))}
              </tbody>
            </table>
